...
import cl.utfsm.transchantiago.util.Settings;
...
public class GameScreen extends GameCanvas implements Runnable, CommandListener {
  ...
  private boolean noKeyPressed = true;
    ...
    private Sprite peatonNo;
    private Sprite peatonEs;
    private Sprite peatonLa;

  

  // Constructor and initialization
  public GameScreen(MidletTrans midlet,Settings settings,Score score) throws Exception {
    super(true);
    ...
    Image peatonNormal = Image.createImage("/peatonNormal.png");
    peatonNo = new Sprite(peatonNormal);
    peatonNo.setPosition(303, 237);
    
    Image peatonEscolar = Image.createImage("/peatonEscolar.png");
    peatonEs = new Sprite(peatonEscolar);
    peatonEs.setPosition(645, 235);
    
    Image peatonLadron = Image.createImage("/peatonLadron.png");
    peatonLa = new Sprite(peatonLadron);
    peatonLa.setPosition(303, 236);
    ...
    layerManager.append(peatonNo);
    layerManager.append(peatonEs);
    layerManager.append(peatonLa);
...
/**
    ...
    private void verificaParadero() {
        if(this.gameMap.getisStop()){
            //if(edificios.g){
  ...
  g.drawString("Tiempo :" + this.timeGame, 0, 0, Graphics.LEFT | Graphics.TOP);
    flushGraphics();
  }
