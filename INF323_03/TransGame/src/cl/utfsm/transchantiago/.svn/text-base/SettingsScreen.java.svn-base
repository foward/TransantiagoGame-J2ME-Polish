...
public class SettingsScreen extends Form implements CommandListener {
  ...
  private static final String[] difficultyOptions = {"Facil","Medio","Dificil"};
  private static final String[] autoFireOptions = {"Off","On"};
  private ChoiceGroup difficulty;
  private ChoiceGroup autoFire;


  public SettingsScreen (MidletTrans midlet, Settings settings) throws Exception {
    super("Settings");
    this.midlet = midlet;
    this.settings = settings;
    difficulty = new ChoiceGroup("Dificultad", ChoiceGroup.POPUP, difficultyOptions,null);
    autoFire = new ChoiceGroup("Sonido", ChoiceGroup.POPUP, autoFireOptions,null);
    append(difficulty);
    append(autoFire);
